# Микросервис registration-service
Микросервис для работы с регистрациями на мероприятия.

Данный микросервис позволяет пользователям возможность регистрироваться на мероприятия, а организаторам мероприятия подтверждать или отклонять эти заявки.
...
## Эндпоинты:
• **POST** _/registrations_ - создание регистрации;

• **PATCH** _/registrations_ - обновление заявки;

• **GET** _/registrations/{registrationId}_ - получение регистрации по id;

• **GET** _/registrations?page={page}&size={size}&eventId={eventId}_ - получение списка регистраций с пагинацией и с обязательным указанием id события;

• **DELETE** _/registrations_ - удаление регистрации (по связке номера + пароля из dto);

• **PATCH** _/{registrationId}/status_ -  одобрение/отклонение решистрации организаторами события;

• **PATCH** _/{registrationId}/status/decline_ - отклонение решистрации организаторами события;

• **GET** _/registrations/search_ - получение заявки по заданным статусам (одному или более) и по id события, отсортированными по дате создания;

• **GET** _/registrations/count_ - возвращает количество заявок в разных статусах по eventId.

## Запустить микросервис можно двумя способами.
### Первый способ:
•	Предварительно создать БД _POSTGRES_ не ниже версии 15 с именем "_registrations-db_".
...
•	Запустить микросервис командой `java -jar registration-service-1.0.0-SNAPSHOT.jar`.

•	Микросервис будет доступен по адресу: _http://localhost:8090_.
...
_http://localhost:8090/swagger-ui/index.html_
